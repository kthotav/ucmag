...
def list_posts_by_datetime():
    ...
    query = db.post.category==category.id
...
def view_post():
    ...
    if auth.user:
        ...
        db.post.uc.requires=IS_IN_SET(['UC Berkeley', 'UC Davis', 'UC Irvine', 'UC Los Angeles', 'UC Merced', 'UC Riverside', 'UC San Diego', 'UC Santa Barbara', 'UC Santa Cruz'])
        db.post.category.writable = True
        db.post.category.requires = IS_IN_DB(db,'category.id','%(name)s')
        form_edit_post = SQLFORM(db.post, id, showid=False, fields=['uc', 'category', 'title', 'body'], submit_button='Update').process(formname='edit_body_form', next='view_post/[id]')
...
def get_vote_icon (user_id, post_id):
    ...
    if user_id == 0 or post_id == 0:
        ...
        vote_icon = A('', _class="fa fa-thumbs-o-up fa-fw vote-link", callback=URL('vote', vars={"post_id":post_id}), target='spanVote')
    else:
        vote_icon = A('', _class="fa fa-thumbs-up fa-fw vote-link", callback=URL('vote', vars={"post_id":post_id}), target='spanVote')
...
def get_fav_icon (user_id, post_id, is_ms=False):
    ...
    if user_id == 0 or post_id == 0:
        ...
        fav_icon = A('', _class="fa fa-star-o fa-fw fav-link", _href=URL('favourite', vars={"post_id":post_id, "is_ms":is_ms}) )
    elif likes == 0 and not is_ms:
        fav_icon = A('', _class="fa fa-star-o fa-fw fav-link", callback=URL('favourite', vars={"post_id":post_id}), target='spanFav')
    elif likes > 0 and is_ms:
        fav_icon = A('', _class="fa fa-star fa-fw fav-link", _href=URL('favourite', vars={"post_id":post_id, "is_ms":is_ms}) )
    else:
        fav_icon = A('', _class="fa fa-star fa-fw fav-link", callback=URL('favourite', vars={"post_id":post_id}), target='spanFav')
...
def delete_post():
    ...
    if auth.user and post and auth.user.id == post.created_by:
        ...
        if request.env.HTTP_REFERER and request.env.HTTP_REFERER.find('stories'):
